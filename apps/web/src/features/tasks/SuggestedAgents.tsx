...
function PostCreateJourney({
  ...
  const needs = requiredToolkits({ kind: s.kind, instructions: s.instructions });
    ...
    return (
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-muted-foreground text-xs">
          One step left: connect {names} so it can do its job. Takes about 30 seconds.
        </p>
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <p className="flex items-center gap-1.5 text-xs font-medium text-emerald-600">
        <Check className="size-3.5" /> {needs.length > 0 ? "Connected and ready" : "Agent created"}
      </p>
      {run.isError && (
        <p className="text-destructive text-xs">
          {(run.error as Error)?.message || "The run failed. Try again."}
        </p>
      )}
      <div className="flex flex-wrap items-center gap-1.5">
        <Button size="sm" disabled={run.isPending} onClick={() => run.mutate(agentId)}>
          {run.isPending ? (
            ...
            <>
              <Sparkles className="size-3.5" /> Run it now
            </>
          )}
        </Button>
        {viewAgentLink}
