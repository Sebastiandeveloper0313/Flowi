...
import { CalendarClock, Check, Loader2, PenLine, Radar, RefreshCw, Sparkles } from "lucide-react";
 ...
 * dashboard proposes real work instead of a blank chat.
 */
export function SuggestedAgents() {
  ...
  const teamId = ws?.id;
  const enabled = Boolean(teamId && ws?.business_context) && !tasksLoading && tasks?.length === 0;
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {(suggestions ?? []).map((s) => (
            <SuggestionCard
              ...
              suggestion={s}
              teamId={teamId!}
              onCreated={() => queryClient.invalidateQueries({ queryKey: taskKeys.all })}
            />
          ))}
        </div>
      )}
      ...
      <CardContent className="flex flex-1 flex-col gap-2 p-5">
        ...
        {createdId ? (
          <Button variant="outline" size="sm" asChild>
            <Link to="/agents/$agentId" params={{ agentId: createdId }}>
              <Check className="size-4 text-emerald-600" /> Created · view agent
            </Link>
          </Button>
        ) : (
