...
export function AgentStatusBadge({ status }: { status: AgentStatus }) {
  const map: Record<AgentStatus, { label: string; cls: string; dot: string }> = {
    running: { label: "Running", cls: "bg-emerald-50 text-emerald-700 border-emerald-200", dot: "bg-emerald-500" },
    paused: { label: "Paused", cls: "bg-slate-100 text-slate-600 border-slate-200", dot: "bg-slate-400" },
    needs_input: { label: "Needs input", cls: "bg-amber-50 text-amber-700 border-amber-200", dot: "bg-amber-500" },
  };
  const s = map[status];
  return (
    <span
