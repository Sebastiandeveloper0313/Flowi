...
/**
 * The employee's desk: everything Sentrive is doing for this business in one
 * place. One feed of finished work, one stack of things waiting on the user,
 * and the skills it's currently running. The agents underneath are unchanged;
 * this is the single front door so users stop checking N separate pages.
 */
function DeskPage() {
  ...
  return (
    <div className="flowy-page">
      {/* The employee, not a toolbox: one identity doing all of it. */}
      <header className="mb-7 flex flex-wrap items-center gap-4">
        ...
        <Button asChild size="sm">
          <Link to="/dashboard" search={{ c: undefined }}>
            <MessageSquare className="size-4" /> Give it something to do
          </Link>
        </Button>
      </header>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatTile icon={ArrowUpRight} label="Replies posted · 24h" value={stats?.postedReplies} />
        <StatTile
          icon={Activity}
              ...
              {loading ? (
                ...
                <div className="-mx-2">
                  {runs.slice(0, 12).map((run) => (
                    <FeedRow key={run.id} run={run} title={titleById.get(run.task_id) ?? "Task"} />
                  ))}
                </div>
              )}
            </CardContent>
            ...
            <CardContent className="space-y-2">
              {tasksLoading ? (
                ...
                <p className="text-muted-foreground py-4 text-sm">
                  No skills yet. Tell Sentrive what to handle and it shows up here.
                </p>
              ) : (
