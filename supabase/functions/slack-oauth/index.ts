// Entrives - Slack install (OAuth). Two jobs:
...
// to their own Entrives account by email in slack-events.
import { createClient } from "jsr:@supabase/supabase-js@2";

// im:write lets Entrives open a DM to the user proactively (approval pings).
...
Deno.serve(async (req: Request) => {
  ...
  if (!code) {
    ...
    // Start the install: send the user to Slack's consent screen. The Entrives
  ...
  // The Entrives team that started the install (echoed back by Slack in `state`).
  const state = url.searchParams.get("state") ?? "";
