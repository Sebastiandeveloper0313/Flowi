// Sentrive - Slack interactivity. Handles the Approve / Reject buttons on the
// approval notifications Sentrive DMs into Slack. Authorized two ways: the
// request must carry a valid Slack signature, and the clicking Slack user is
// email-matched to a Sentrive account that must belong to the approval's team.
...
async function handleAction(payload: any): Promise<void> {
  ...
  if (!token) return respond(responseUrl, "This Slack workspace isn't linked to Sentrive anymore.");
  ...
  // Email -> Sentrive user -> must belong to the approval's team.
  const { data: users } = await admin.auth.admin.listUsers({ page: 1, perPage: 1000 });
  const flowyUser = (users?.users ?? []).find(
    (u: { email?: string }) => (u.email ?? "").toLowerCase() === email.toLowerCase(),
  );
  if (!flowyUser) return respond(responseUrl, `No Sentrive account for ${email}.`);
