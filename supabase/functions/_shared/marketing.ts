...
/** How much Senable may do on its own. 'ask' is the safe default. */
...
export function operatorPersona(ws: WorkspaceContext | null): string {
  return (
    `You are Senable, a senior marketing and distribution operator working inside ${companyName(ws)}. ` +
...
export function runnerSystem(ws: WorkspaceContext | null): string {
  return (
    ...
    "- Do not narrate your process. Reply with the finished deliverable only. Senable delivers it to the user's chosen " +
