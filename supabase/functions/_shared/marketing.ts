...
export function chatSystem(ws: WorkspaceContext | null): string {
  return (
    ...
    '- SEO blog writing: kind "seo_blog". Each run writes a complete, SEO-optimized article for the business\'s website (title, meta description, structured body). If they connect WordPress on the Integrations page, each article lands straight in their blog: as a WordPress draft to review, or published live when the agent is on auto. Without WordPress it lands in the app to review and paste. Use this whenever the user wants recurring blog posts, articles, or SEO content.\n' +
