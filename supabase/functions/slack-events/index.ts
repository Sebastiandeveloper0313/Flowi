// Sentrive - Slack bot. DM Sentrive (or @mention it) in Slack and it does real work:
// same operator brain, same connected tools, same approval gate as the web chat.
// The Slack user is matched to their Sentrive account by email (users:read.email),
// so there is no linking flow: if your Slack email has a Sentrive account, it works.
...
/**
 * Recent turns of a DM with Sentrive, oldest first, so follow-ups have context.
...
/** Find the Sentrive team for an email address (service role; matched case-insensitively). */
// deno-lint-ignore no-explicit-any
...
// deno-lint-ignore no-explicit-any
async function runSentrive(admin: any, teamId: string, convo: Turn[]): Promise<string> {
  const key = Deno.env.get("ANTHROPIC_API_KEY");
  if (!key) return "Sentrive isn't configured on the server yet.";
  ...
  const system =
    ...
    "Sentrive dashboard. Everything else works: answer, use the connected tools, and take actions per " +
...
async function handleEvent(event: any, slackTeamId: string | undefined): Promise<void> {
  ...
  if (!email) {
    await post("I couldn't read your Slack email, so I can't find your Sentrive account.");
    return;
  }
  ...
  try {
    const reply = await runSentrive(admin, teamId, convo);
    await post(reply);
  } catch (e) {
